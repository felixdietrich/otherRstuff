...
#---------------------------------------------------------
#TODO: try different time intervals!!!

#apply 15min, 30min, 60min etc
    ...
    if(tsignal[j,i]==-1) {  
      ...
      if(nrow(pos_ec_long) < 2) {
        pos_ec_short=rbind(coredata(P[j,i]),pos_ec_short)
          ...
          balance <- rbind(balance,coredata(P[j,i])-pos_ec_long[1,1])        
...
#---------------------------------------------------------
#sum up account balance
result <- sum(balance)-88
